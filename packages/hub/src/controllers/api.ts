...
export default class API {
    ...
    private handleRouter(router: Router): void {
        ...
        router.get('/leaderboards/new', this.handleNewLeaderboards.bind(this));
    ...
    private handleNewLeaderboards(request: Request, response: Response): void {
        this.setHeaders(response);

        let combatSkills = [1, 2, 3, 4, 6, 7],
            combatExpPromises = combatSkills.map((skill) => {
                return this.cache.getSkillsExperienceWithoutCb(skill);
            }),
            totalExperiencePromise = this.cache.getTotalExperienceWithoutCb();

        Promise.all([...combatExpPromises, totalExperiencePromise])
            .then((results) => {
                let totalExperience = results.pop(),
                    combatExp = results;
