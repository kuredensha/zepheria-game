...
export default class Cache {
    ...
    public async getTotalExperienceWithoutCb(): Promise<TotalExperience[] | undefined> {
        if (!this.canAggregateData(this.lastAggregates.total)) return this.totalExperience;

        return new Promise<TotalExperience[] | undefined>((resolve, reject) => {
            this.database.getTotalExperienceAggregate((data: TotalExperience[]) => {
                try {
                    for (let info of data) if (!info.cheater) delete info.cheater;

                    this.totalExperience = data;

                    // Update the last aggregate time.
                    this.lastAggregates.total = Date.now();
                    resolve(this.totalExperience);
                } catch (error) {
                    reject(error);
                }
            });
        }).catch((error) => {
            console.error('Failed to get total experience:', error);
            return [];
        });
    }
    ...
    public async getSkillsExperienceWithoutCb(
        skill: Modules.Skills
    ): Promise<SkillExperience[] | undefined> {
        if (!this.canAggregateData(this.lastAggregates[skill])) return this.skillsExperience[skill];

        return new Promise<SkillExperience[] | undefined>((resolve, reject) => {
            this.database.getSkillAggregate(skill, (data: SkillExperience[]) => {
                try {
                    for (let info of data) if (!info.cheater) delete info.cheater;

                    this.skillsExperience[skill] = data;

                    // Update the last aggregate time.
                    this.lastAggregates[skill] = Date.now();
                    resolve(this.skillsExperience[skill]);
                } catch (error) {
                    reject(error);
                }
            });
        }).catch((error) => {
            console.error('Failed to get skill experience:', error);
            return [];
        });
    }
