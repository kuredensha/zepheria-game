...
import { getAddress, connectMetamask, isAddressValid } from './utils/metamask';
...
export default class App {
    ...
    private addressField: HTMLInputElement = document.querySelector('#address-input')!;
    ...
    private connectButton: HTMLButtonElement = document.querySelector('#connect-wallet')!;
    ...
    private load(): void {
        ...
        // Connect Wallet buttn
        this.connectButton.addEventListener('click', this.connectWallet.bind(this));
    }
    ...
    public openScroll(destination: string): void {
        ...
        this.clearErrors();
        this.connectButton.toggleAttribute('hidden', destination === 'load-character');
        ...
        connectMetamask().then((connected) => {
            if (connected)
                getAddress().then((result) => {
                    if (result) this.addressField.value = result.address;
                    else this.sendError('Could not get address from MetaMask');
                });
            else this.sendError('Could not connect to MetaMask');
        });
    }
    ...
    private verifyForm(): boolean {
        ...
        if (this.isRegistering()) {
            ...
            // Verify address.
            if (!isAddressValid(this.getAddress()))
                return this.sendError(
